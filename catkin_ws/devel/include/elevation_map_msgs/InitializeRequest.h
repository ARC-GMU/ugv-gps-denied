...
struct InitializeRequest_
{
  ...
  _method_type method;
   ...
   typedef std::vector< ::geometry_msgs::PointStamped_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::geometry_msgs::PointStamped_<ContainerAllocator> >> _points_type;
  _points_type points;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::elevation_map_msgs::InitializeRequest_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.points.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::PointStamped_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.points[i]);
    }
    if (v.points.empty() || false)
      s << "]";
  }
